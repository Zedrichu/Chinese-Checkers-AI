...
def find_circle(mouse):
    print(f'mouse: {mouse}')
    x, y = (mouse - OFFSET) // CELL_SIZE

    if ((x * CELL_SIZE - CIRCLE_RADIUS < mouse[0] - OFFSET < x * CELL_SIZE + CIRCLE_RADIUS) and
            (y * CELL_SIZE - CIRCLE_RADIUS < mouse[1] - OFFSET < y * CELL_SIZE + CIRCLE_RADIUS)):
        return x, y

    print("Clicked outside of circle")
    return None
            ...
            for j in range(self.board.board_size):
                if self.board.board[i][j] == 1:
                    pg.draw.circle(screen, (0, 0, 255),
                                   (j * CELL_SIZE + OFFSET, i * CELL_SIZE + OFFSET), CIRCLE_RADIUS)
